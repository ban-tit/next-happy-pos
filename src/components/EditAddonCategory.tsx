...
import { getAccessToken, getAddonCategoriesByLocationId, getMenuByLocationId, getMenuCategoriesByLocationId, getMenusByAddonCategoryId, getSelectedLocationId } from "../utils";
...
const EditAddonCategory= ()=>{
    ...
    const {menus, menuCategories, menusAddonCategories, addonCategories, locations, menusMenuCategoriesLocations, fetchData} = useContext(AppContext)
    const params = useParams()
    const [open, setOpen] = useState(false)
    const [openDeleteAddonCategoryDialog, setOpenDeleteAddonCategoryDialog] = useState(false)
    ...
    const addonCategory = addonCategories.find(item => item.id === Number(addonCategoryId))
    if(!addonCategory) return <Layout title="Edit Addon Category">
                                <Box
...
const menuCategorieByLocationId = getMenuCategoriesByLocationId(menuCategories, menusMenuCategoriesLocations)
console.log('menuCategorieByLocationId', menuCategorieByLocationId);
const chosenMenuIds = menusAddonCategories.filter(item => item.addon_categories_id === Number(addonCategoryId)).map(item => item.menus_id);

console.log('test', menus.map(item => chosenMenuIds.includes(item.id as number)));
    ...
    const mappedMenus = getMenuByLocationId(menus, menusMenuCategoriesLocations).map(item => ({id: item.id as number, name: item.name}))
    ...
    const menusByAddonCategoryId = getMenusByAddonCategoryId(Number(addonCategoryId), menusAddonCategories)
    ...
    const mappedValidMenus = mappedMenus.filter(item => validMenuIds.includes(item.id))
    ...
    const updateAddonCategory = async () => {
        ...
        if(!newAddonCategory.name.length){
            newAddonCategory.name = addonCategory.name
        }
    ...
    const handleRemoveMenuFromMenuCategory = async () => {
        await fetch(`http://localhost:5000/menuCategories/removeMenu`,{
            ...
            body: JSON.stringify({menuId: selectedMenu?.id, addonCategoryId, locationId: selectedLocationId})
        })
        accessToken && fetchData(accessToken)
    }
            ...
            <Box sx={{p: 3}}>
                ...
                <Box>   
                    ...
                    <Autocomplete 
                        ...
                        label='Menus' 
                        placeholder="Menus"
                        onChange={(options)=>setNewAddonCategory({...newAddonCategory, menuIds: options.map(item => item.id)})}/>
